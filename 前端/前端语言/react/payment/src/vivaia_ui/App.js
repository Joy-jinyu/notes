...
import { NoPayment, Cards, RenderCardImg, ApplePay, HeaderTop } from "../compoents/apps"
import { Navigator, StepTitle, ShippingAddress, ShippingMethed, OrderSummary, AboutUs, MobileOrder, Logo } from "../compoents/vivaiaui"
...
class Index extends Component {
    ...
    get_harbor_shipping_method = async () => {//shopify之外的类型获取邮费列表
        ...
        shippingMethods && shippingMethods.length > 0 && shippingMethods.forEach(item => {
            shipping_method_list.push({ shipping_rate_id: `${item.id}`, price: item.price || 0, name: item.methodName || '', describe: item.describe || '', originPrice: item.originPrice || 0 });
        })
    ...
    onsubmitVerifyFun = (hideError = false, noVerifyArr) => {//专门用来验证参数,hideError:是否隐藏校验的错误提示
        ...
        const isGALAXY = (platform_name === "GALAXY");//GALAXY不验证
        ...
        if (isGALAXY) {//如果是GALAXY，就只验证cpf
            for (let key in formData) {
                if (!noVerifyArr.includes(key)) {
                    ...
                    cpfItme.forEach((item) => {//eslint-disable-line
    ...
    post_checkout_deal = async (finalData, btnType) => {//最后提交
        ...
        checkout_deal(finalData).then(async (res = {}) => {
            ...
            this.setState({ okText, sub_error_obj: err || {}, paymentAlert, subLoading: false, visible: false });
            if (!status) {
    ...
    FeedbackDom = () => {
        const { formData } = this.state;
        return <div className={styles.feedback}><Feedback formData={formData} callback={this.returnLink} /></div>
    }
    render() {
        const { t } = this.props;
        const { credits, okText, gatewayList = [], paymentLoadError = false, instalmentsItem, modalVisible, modalText, sub_error_obj = {}, cpfItme, subDisabled, onGateWayItem = {}, paymentAlert, gatewayCountryLoading = false, pay_type = 0, formItme, formData, visible, shipping_method_list = [], subLoading, simplifyList = [], isSimplify = false, shippingMethodLoading = false } = this.state;
        ...
        let { order = {}, shop = {} } = window.shopify_checkouts || {};
        let { expiredAt, items = [], discounts, extras = [], orderNumber, items_subtotal_price } = order || {};
        ...
        let subBtnText = isMobile ? (isApplePay ? <span><Sicon className={styles.app_btn_icon} icon='appBtn' />{`(${viewPrice})`}</span> : <><span className={styles.btn_price}>{t('payNow')}{`(${viewPrice})`}</span></>) : (isApplePay ? <span><Sicon className={styles.app_btn_icon} icon='appBtn' /></span> : <span className={styles.btn_price}>{t('payNow')}</span>);
        let renderBtn = () => {
        ...
        let discountsList = () => {
            const hasPrice = this.getPostageData('price') && ((this.getPostageData('price') * 1) > 0);
            return <>
                {cpfDom()}
                <div className={styles.rightItem}>
                    ...
                    <div className={classNames(styles.subtotal)}>
                        ...
                        <span className={classNames(styles.text, !hasPrice && styles.Free)}>{hasPrice ? currencySequence(this.getPostageData('price') || '') : t('Free')}</span>
                    </div>
                    {chargeList()}
        ...
        const mobileDom = () => {
            return <div className={styles.mo_app} id='mo_app'>
                {!IS_APP && <div className={styles.back}><Sback SmodalCancel={this.returnLink} onClick={this.beforReturnLink} title={t('Checkout')} /></div>}
                ...
                {gatewayList && gatewayList.length > 0 ? <div className={styles.mo_main} ref={e => this.mo_main = e} style={IS_APP ? {} : { paddingTop: '3.062rem' }}>
                    <div className={styles.mo_main_top}>
                        <div className={styles.Navigator}><Navigator /></div>
                        <MobileOrder parentState={this.state} isMIDDLE={this.isMIDDLE} onGateWayItem={onGateWayItem} items={items || []} formData={formData} shop={shop} cpfItme={cpfItme}>{discountsList()}</MobileOrder>
                    ...
                    <div className={styles.mo_feedback_box}>{this.FeedbackDom()}</div>
                    <div className={styles.payNow} ref={e => this.payNow = e} style={IS_APP ? { paddingBottom: 25 } : {}}>
                        {countDownDom}
                        {renderBtn()}
                        <div className={styles.secureAndEncrypted}>
                            <svg width="10" height="12" viewBox="0 0 10 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M0.5 4.5H9.5V12H0.5V4.5ZM2 6V10.5H8V6H2ZM2 6L0.5 6V4.5H2V3C2 1.35 3.35 -1.75781e-08 5 0C6.65 1.75781e-08 8 1.35 8 3V4.5H9.5V6L6.5 6V3C6.5 2.175 5.825 1.5 5 1.5C4.175 1.5 3.5 2.175 3.5 3V6H2ZM5 9C4.55 9 4.25 8.7 4.25 8.25C4.25 7.8 4.55 7.5 5 7.5C5.45 7.5 5.75 7.8 5.75 8.25C5.75 8.7 5.45 9 5 9Z" fill="black" fillOpacity="0.5" />
                            </svg>
                            {t('vivaia.secureAndEncrypted')}
                        </div>
                    </div>
                </div> : <NoPayment />}
            </div>
        ...
        const pcDom = () => {
            return gatewayList && gatewayList.length > 0 ? <div className={styles.app}>
                <div className={styles.left}>
                    <div className={styles.leftMain}>
                        <Logo />
                        <StepTitle step={1} title={t('shippingAddress.label')} />
                        <ShippingAddress formData={formData} shop={shop} />
                        <StepTitle step={2} title={t('shippingMethod.label')} />
                        <ShippingMethed item={(shipping_method_list && shipping_method_list[0]) || {}} />
                        <StepTitle step={3} title={t('paymentMethod.label')} rightMain={this.FeedbackDom()} />
                        <Salert okText={okText} theme={display_type === 1 ? 'modal' : ''} className={styles.Salert} type='error' value={paymentAlert} onOk={this.clearAlert} />
                        {this.render_gate_way()}
                        <div className={styles.wrap}>
                            <RenderCardImg icon={'pcis'} />
                            <p>{t('paymentMethod.completelyAndSecure')}</p>
                        </div>
                        <div className={styles.Copyright}>{t('vivaia.Copyright')}</div>
                        <div className={styles.CardIconParent}><RenderCardImg icon={'support'} className={styles.CardIcon} /></div>
                    </div>
                </div>
                <div className={styles.right}>
                    <div className={styles.rightMain}>
                        ...
                        <div className={styles.aboutUsBox}><AboutUs /></div>
                    </div>
                </div>
            </div> : <NoPayment />
        }
